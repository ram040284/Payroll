...
public class PaybillPdfRep extends PdfBuilder{
	...
	public void paybillReport(Document doc, PaybillDetails paybillDetails, String imgPath){
		try{
	        ...
	        titleText.append(Utils.getMonthYear(date));
	        ...
	        paybillTab.addCell(addToCell(Utils.getSimpleDate(date), headHdFont));
	        doc.add(paybillTab);
	        doc.add(dottedline);
	...
	private void addTotalDetails(PdfPTable table, PaybillDetails payroll, Font font){
		table.addCell(addToCell(PdfUtils.BASICS, font));
	...
	private void addDeductionTotals(PdfPTable table, PaybillDetails payroll, Font font){
