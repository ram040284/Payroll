...
public class EmpVarDeductionsDAO {
		...
		Session session = null;
			try{
				String queryString = " select new com.payroll.employee.deductions.dataobjects.EmpVarDeductions(d.employee.employeeId, "
						...
						+" d.afkRent, d.society, pfLoanRecovery, d.otherDeductions, d.miscRecovery, d.monthDate,d.note, d.incomeTax)"
	...
	public EmployeeVarDeductions getEmployeeVarDeductions(int empId){
		...
		Session session = null;
			try{
				String queryString = "select new com.payroll.employee.deductions.dataobjects.EmployeeVarDeductions(s.employeeId, s.afkRent, s.society, s.otherDeductions,s.miscRecovery, s.monthDate, s.incomeTax) from EmpVarDeductionsVO s where s.employeeId = ? and s.status = ?";		
